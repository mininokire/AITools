...
def download_instagram_user(username, login_user=None, login_password=None):
    """
    下载指定Instagram用户的所有图片和视频。
    
    :param username: Instagram 用户名
    :param login_user: 如果要下载私密账户的内容，需要提供Instagram登录用户名
    :param login_password: Instagram登录密码
    """
    try:
        # 如果需要下载私密账户内容，先登录
        if login_user and login_password:
            L.login(login_user, login_password)
            print(f"登录成功，下载私密账户内容。")
...
# 示例：下载公开账户内容
download_instagram_user("kirari_1016_")

# 示例：下载私密账户内容，需要登录Instagram
# download_instagram_user("private_user", login_user="your_username", login_password="your_password")
